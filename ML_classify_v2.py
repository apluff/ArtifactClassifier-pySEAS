...
if __name__ == '__main__':
    ...
    try:
        ...
        classifier_pkl = classifier.replace('.hdf5', '_classifier.pkl')
        if os.path.exists(classifier_pkl):
            ...
            raise Exception("Classifier pickle file not found")
    ...
    if args['train']:
        ...
        #Logistic Regression
        logreg = LogisticRegression(C  = 8.25, solver = 'newton-cg', max_iter = 100) # Changed from the original, unusual value of 17.25
        ...
        svm_clf = SVC(kernel="rbf", gamma=0.7, C=8, probability= True) # Removed an erroneous degree argument as it is not used with RBF kernels
        ...
        # Save the metadata to HDF5
        g.save({'datafiles':paths,
            'data - time': dt,
            'domain_keys': domain_vars})

        # Save the classifier separately as a pickle file
        import pickle
        classifier_pkl = classifier.replace('.hdf5', '_classifier.pkl')
        with open(classifier_pkl, 'wb') as f:
            pickle.dump(rnd_clf, f)
        print(f"Saved classifier to: {classifier_pkl}")
