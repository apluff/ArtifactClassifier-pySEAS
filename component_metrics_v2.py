        ...
        for path in paths:
            ...
            if path.endswith('.hdf5'):
                ...
                if (os.path.exists(savepath) and args['force']) or (not os.path.exists(savepath)) or group:
                    ...
                    # === CHANGE #1: BACKWARD COMPATIBILITY FOR 'flipped' ===
                    # Original code: flipped = f.load('flipped')
                    # Now handles missing 'flipped' array for older files
                    if 'flipped' in f.keys():
                        flipped = f.load('flipped')
                    else:
                        print("Note: 'flipped' array not found (older file format). Reconstructing...")
                        # Load eig_vec first to reconstruct flipped
                        ...
                        print(f"Reconstructed flipped array: {np.sum(flipped == 1)} positive, "
                              f"{np.sum(flipped == -1)} negative components")
                    # === END CHANGE #1 ===
                    ...
                    # Load eig_vec if not already loaded
                    if 'eig_vec' not in locals():
                        eig_vec = f.load('eig_vec')
                    
                    # === CHANGE #2: BACKWARD COMPATIBILITY FOR 'thresholds' ===
                    # Original code: thresh = f.load('thresholds')
                    # Now handles missing 'thresholds' array for older files
                    if 'thresholds' in f.keys():
                        thresh = f.load('thresholds')
                    else:
                        print("Note: 'thresholds' array not found (older file format). Computing thresholds...")
                        # Calculate thresholds based on the eigenvectors
                        # n_components should already be defined from the flipped reconstruction
                        if 'n_components' not in locals():
                            n_components = eig_vec.shape[1]
                        
                        thresh = np.zeros(n_components)
                        for i in range(n_components):
                            # Dynamic threshold based on the distribution of component values
                            comp_values = eig_vec[:, i]
                            # Use 2 standard deviations above mean as threshold
                            thresh[i] = np.mean(np.abs(comp_values)) + 2 * np.std(np.abs(comp_values))
                        
                        print(f"Computed thresholds for {n_components} components")
                    # === END CHANGE #2 ===
